...
for i in range(1,lastrow+1):
    ...
    #數值越細 等級越大
    Banding_table['Agility'] = [0, 11.03, 12.10, 13.18, 14.25]
    Banding_table['Power'] = [0, 3.36, 3.86, 4.36, 4.86]
    Banding_table['Coordination'] = [0, 12.47, 13.74, 15.00, 16.27]
    #數值越大 等級越大
    Banding_table['Flexibility'] = [0, 13, 15, 17, 19]
    Banding_table['Endurance'] = [0, 22, 24, 26, 28]
...
for xName in name_radar[0:3]:
  ...
  for rowNum in range(0,last_row):
    ...
    for numB in range(0, 5):
      ...
      if df_radardata.loc[rowNum, xName] > df_Banding_table.loc[numB,xName]:

        df_radardata.loc[rowNum, xName + '.1'] = numB + 1

        #start to reverse 5=1 4=2 3=3 2=4 1=5 ##
        #start to reverse 5=1 4=2 3=3 2=4 1=5 ##
                    
        if df_radardata.loc[rowNum, xName + '.1'] == 5:

          df_radardata.loc[rowNum, xName + '.1'] = 1

        elif df_radardata.loc[rowNum, xName + '.1'] == 4:

          df_radardata.loc[rowNum, xName + '.1'] = 2

        elif df_radardata.loc[rowNum, xName + '.1'] == 3:

          df_radardata.loc[rowNum, xName + '.1'] = 3                  

        elif df_radardata.loc[rowNum, xName + '.1'] == 2:

          df_radardata.loc[rowNum, xName + '.1'] = 4

        else:

          df_radardata.loc[rowNum, xName + '.1'] = 5

# Flexibility and 'Enduranc 數值越大 等級越大

for xName in name_radar[3:5]:
...
# remove all column with all NaN value
df_radardata = df_radardata.dropna(axis=1,how='any')

# convert 1.0 to 1 in Endurance.1 column
df_radardata['Endurance.1'] = df_radardata['Endurance.1'].astype('int') 
...
Banding_table_MuscleMass = {'Band': [1, 2, 3, 4, 5]}
Banding_table_Fat = {'Band': [1, 2, 3, 4, 5]}
selected_columns = ['Trunk Muscle mass', 'LA Muscle mass', 'LL Muscle mass', 'RL Muscle mass', 'RA Muscle mass',
                    'Trunk Fat %', 'LA Fat %', 'LL Fat %', 'RL Fat %', 'RA Fat %']
lastrow_mcData = radar_mcData.shape[0]

for k in range(0,lastrow_mcData):
    radar_mcData['Trunk Muscle mass'][k] = radar_mcData['Trunk Muscle mass'][k]
    radar_mcData['LA Muscle mass'][k] = radar_mcData['LA Muscle mass'][k]
    radar_mcData['LL Muscle mass'][k] = radar_mcData['LL Muscle mass'][k]
    radar_mcData['RL Muscle mass'][k] = radar_mcData['RL Muscle mass'][k]
    radar_mcData['RA Muscle mass'][k] = radar_mcData['RA Muscle mass'][k]

    radar_mcData['Trunk Fat %'][k] = radar_mcData['Trunk Fat %'][k]
    radar_mcData['LA Fat %'][k] = radar_mcData['LA Fat %'][k]
    radar_mcData['LL Fat %'][k] = radar_mcData['LL Fat %'][k]
    radar_mcData['RL Fat %'][k] = radar_mcData['RL Fat %'][k]
    radar_mcData['RA Fat %'][k] = radar_mcData['RA Fat %'][k]

    Banding_table_MuscleMass['Trunk Muscle mass'] = [0, 21.15, 24.99, 28.83, 32.67]
    Banding_table_MuscleMass['LA Muscle mass'] = [0, 1.98, 2.46, 2.94, 3.42]
    Banding_table_MuscleMass['LL Muscle mass'] = [0, 8.64, 10.18, 11.72, 13.26]
    Banding_table_MuscleMass['RL Muscle mass'] = [0, 8.66, 10.22, 11.78, 13.34]
    Banding_table_MuscleMass['RA Muscle mass'] = [0, 2.04, 2.58, 3.12, 3.66]

    Banding_table_Fat['Trunk Fat %'] = [0, 14.65, 20.19, 25.72, 31.27]
    Banding_table_Fat['LA Fat %'] = [0, 13.37, 19.53, 25.69, 31.85]
    Banding_table_Fat['LL Fat %'] = [0, 15.59, 20.67, 25.95, 31.23]
    Banding_table_Fat['RL Fat %'] = [0, 14.65, 20.19, 25.73, 31.27]
    Banding_table_Fat['RA Fat %'] = [0, 12.55, 18.49, 24.43, 30.37]

for selectItem in selected_columns[0:5]:
    
  for rowNum01 in range(0, lastrow_mcData):

    for numB01 in range(0, 5): 

      if radar_mcData[selectItem][rowNum01] >= Banding_table_MuscleMass[selectItem][numB01]:

        radar_mcData[selectItem.replace('mass', 'score')][rowNum01] = numB01 + 1


### Banding_table_Fat Radar Chart ####
### Banding_table_Fat Radar Chart ####
### Banding_table_Fat Radar Chart ####

for selectItem in selected_columns[5:11]:

  for rowNum01 in range(0, lastrow_mcData):

    for numB01 in range(0, 5): 

      if radar_mcData[selectItem][rowNum01] >= Banding_table_Fat[selectItem][numB01]:
        
        radar_mcData[selectItem+' '+'score'][rowNum01] = numB01 + 1

        if radar_mcData[selectItem+' '+'score'][rowNum01] == 5:

          radar_mcData[selectItem+' '+'score'][rowNum01] = 1

        elif radar_mcData[selectItem+' '+'score'][rowNum01] == 4:

          radar_mcData[selectItem+' '+'score'][rowNum01] = 2

        elif radar_mcData[selectItem+' '+'score'][rowNum01] == 3:

          radar_mcData[selectItem+' '+'score'][rowNum01] = 3                  

        elif radar_mcData[selectItem+' '+'score'][rowNum01] == 2:

          radar_mcData[selectItem+' '+'score'][rowNum01] = 4

        else:

          radar_mcData[selectItem+' '+'score'][rowNum01] = 5

mcData = radar_mcData



######################################Data frame of scatter plot
...
index_page = html.Div([
    ...
    html.Br(),
    html.Div([
            html.H6(children='Page1 is IVE student health graph', 
            style={
                'font-size': '35px',
                'color': colors['text']})
    ...
    style={
        ...
        'background-image':'url("/assets/IVE_Chai_Wan_Campus_overview_2018.jpg")',
...
page_1_layout = html.Div([
    html.Div([
        ...
        ##########creeate Student WEIGHT distribution
                ...
                ], className='create_container2 three columns', style={'height': '550px', 'width': '650px'})
...
page_2_layout = html.Div([
    ...
    html.Div([
        html.H6(children='Please Input your Fat%',
                        style={'color': '#ee82ee', 'font-size':'20px'}),
        html.Div(dcc.Input(id='Fat_input', type='text')),
        html.Br(),
        html.H6(children='Please Input your TBW%',
                        style={'color': '#ee82ee', 'font-size':'20px'}),
        html.Div(dcc.Input(id='TBW_input', type='text')),
        html.Br(),
        html.H6(children='Please Input your BMI',
                        style={'color': '#ee82ee', 'font-size':'20px'}),
        ...
        html.Br(),
        html.Br(),
        html.Div(id = "model",style={'color': colors['text'], 'font-size':'30px'})
    ]),
...
def update_output(n_clicks, Fat_input, TBW_input, BMI_input):
    new_data= []
    new_data = [[Fat_input, TBW_input, BMI_input]]
